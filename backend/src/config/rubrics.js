const RUBRICS = {
  '9': { // NIVEL A — Explorador
    name: 'Explorador',
    profile: 'Está aprendiendo a ver problemas como estructuras, no como ejercicios. Necesita guía frecuente.',
    expectations: 'Comprender ideas centrales, identificar variables',
    dimensions: {
      'representacion': {
  ...
  '10': { // NIVEL B — Analista
    name: 'Analista',
    profile: 'Empieza a organizar su pensamiento. Puede trabajar con menos guía.',
    expectations: 'Explicar supuestos, seguir razonamiento',
    dimensions: {
      'representacion': {
        name: 'Representación y modelación',
  ...
  '11': { // NIVEL C — Constructor
    name: 'Constructor',
    profile: 'Piensa de forma estructurada y consciente. Se aproxima al nivel universitario inicial.',
    expectations: 'Cuestionar modelos, proponer alternativas',
    dimensions: {
      'representacion': {
        name: 'Representación y modelación',
  ...
  'graduate': { // NIVEL D — Estratega
    name: 'Estratega',
    profile: 'Opera con alto nivel de autonomía y rigor. Entra en lógica universitaria y de pruebas exigentes.',
    expectations: 'Generalizar, criticar, transferir entre dominios',
    dimensions: {
      'representacion': {
        name: 'Representación y modelación',
...
const LEVEL_DESCRIPTORS = {
  1: 'Inicial - Respuestas intuitivas, poco estructuradas',
  2: 'En desarrollo - Uso parcial de estrategias',
  3: 'Competente - Uso consciente y consistente',
  4: 'Avanzado - Uso flexible, crítico y transferible'
};

// Map bitacora sections to cognitive dimensions
const SECTION_TO_DIMENSION = {
  'observaciones': 'representacion',
  'preguntas': 'metacognicion',
  'hipotesis': 'abstraccion',
  'variables': 'representacion',
  'experimentos': 'estrategia',
  'errores_aprendizajes': 'metacognicion',
  'reflexiones': 'metacognicion',
  'conclusiones': 'transferencia'
};

function getRubricCriteria(gradeLevel, dimension, level) {
  const rubric = RUBRICS[gradeLevel];
  if (!rubric) return null;
  
  const dimensionData = rubric.dimensions[dimension];
  if (!dimensionData) return null;
  
  const levelName = LEVEL_NAMES[level];
  return dimensionData[levelName];
}

function getGradeProfile(gradeLevel) {
  return RUBRICS[gradeLevel] || RUBRICS['9'];
}

function getDimensionForSection(section) {
  return SECTION_TO_DIMENSION[section] || 'representacion';
}
