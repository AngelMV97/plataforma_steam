    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm border-b border-[#E5E7EB] dark:border-[#1F2937]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <button
            onClick={() => router.back()}
            className="text-[#2F6F6D] dark:text-[#4A9B98] hover:text-[#1F4A48] dark:hover:text-[#3A8A87] mb-2 flex items-center gap-2 transition-colors font-medium text-sm sm:text-base"
          >
            ← Volver
          </button>
          <h1 className="text-xl sm:text-2xl font-bold text-[#1F3A5F] dark:text-[#5B8FB9]">
            Evaluar Bitácora
          </h1>
          <div className="mt-2 text-xs sm:text-sm text-[#6B7280] dark:text-[#9CA3AF] space-y-1">
            <p><strong>Estudiante:</strong> {student.full_name} ({student.grade_level}°)</p>
            <p className="line-clamp-1"><strong>Artículo:</strong> {attempt.articles.title}</p>
      ...
      {successMessage && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
          <div className="flex items-center gap-3 p-3 sm:p-4 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-lg border border-green-200 dark:border-green-800 text-xs sm:text-sm">
            <svg className="w-4 sm:w-5 h-4 sm:h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <span className="font-medium line-clamp-2">{successMessage}</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
          <div className="flex items-center gap-3 p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg border border-red-200 dark:border-red-800 text-xs sm:text-sm">
            <svg className="w-4 sm:w-5 h-4 sm:h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 border border-[#E5E7EB] dark:border-[#1F2937]">
            <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#1F3A5F] dark:text-[#5B8FB9] flex items-center gap-2">
              <svg className="w-5 sm:w-6 h-5 sm:h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <span className="hidden sm:inline">Bitácora del Estudiante</span>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 border border-[#E5E7EB] dark:border-[#1F2937] sticky top-4 self-start">
            <h2 className="text-lg sm:text-xl font-semibold mb-4 text-[#1F3A5F] dark:text-[#5B8FB9] flex items-center gap-2">
              <svg className="w-5 sm:w-6 h-5 sm:h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
              </svg>
              <span className="hidden sm:inline">Formulario de Evaluación</span>
